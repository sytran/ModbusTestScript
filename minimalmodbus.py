...
class Instrument:
    ...
    def read_registers(self, registeraddress, number_of_registers, functioncode=3, number_of_values=0, is_float=True):
    ...
    if functioncode in [3, 4]:
        ...
        if payloadformat == _PAYLOADFORMAT_STRING:
            ...
            if number_of_values > 0:
                if is_float:
                    floatArray = []
                    startIndex = _NUMBER_OF_BYTES_BEFORE_REGISTERDATA
                    for _ in range(number_of_values):
                        registerdata = payload[startIndex:startIndex+4]
                        floatArray.append(_bytestring_to_float(registerdata, 2, BYTEORDER_BIG))
                        startIndex += 4
                    return floatArray
                else:
                    intArray = []
                    ...
                    for _ in range(number_of_values):
                        registerdata = payload[startIndex:startIndex+4]
                        intArray.append(_bytestring_to_long(registerdata, False, 2, BYTEORDER_BIG))
                        startIndex += 4
                    return intArray
            else:
